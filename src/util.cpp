...
const char * const BITCOIN_CONF_FILENAME = "folm.conf";
const char * const BITCOIN_PID_FILENAME = "folmd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "folm" is a composite category enabling all Folm-related debug output
            if(ptrCategory->count(string("folm"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\FolmCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\FolmCore
    // Mac: ~/Library/Application Support/FolmCore
    // Unix: ~/.folmcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FolmCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/FolmCore";
#else
    // Unix
    return pathRet / ".folmcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty folm.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override folm.conf
