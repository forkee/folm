...
*Setup instructions for a gitian build of Folm using a Debian VM or physical system.*
...
Clone the git repositories for folm and gitian and then checkout the folm version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the folm repository
...
To build Folm (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the folm repository.
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/folm-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/folm-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/folm-build.assert
```
